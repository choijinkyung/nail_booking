...
### 고객 (로그인 불필요)
- **`/`** 소개·주의사항·가격표·위치
- **`/book`** 시술 선택 → 1지망 + 대체 시간 선택 → 정보 입력(이름 + **전화번호**, 이메일은 선택) → 안내 동의 → 요청
  - 요청 완료 시 **조회 코드**(예: `3F9K2P`)를 받습니다.
- **`/status`** 예약 조회 — **예약번호** 또는 **이름 + 전화번호** 로 확인. 진행 상황(확인 중 / 확정 / 불가) + **변경/취소 요청**
...
- **보안**:
  ...
  - 고객 식별은 **이름 + 전화번호**. 표기가 달라도 맞도록 정규화 키(`contact_norm`)로 비교하고, 화면에는 손님이 입력한 원본을 그대로 보여줍니다.
